...
const MainInfo = ({ discussion }: props) => {
    return (
        <div className='discussion-info' style={{ transition: 'height 0.3s, padding 0.3s'}}><div className='disc-users'>
            <div className='disc-owner'>
                <span>Создатель обсуждения</span>
                <div className='disc-owner--info'>
                    <img src={`${
                        discussion.owner?.avatar
                            ? `http://localhost:5665/api/upload/${encodeURIComponent(discussion.owner.avatar)}` 
                            : '/images/profile.png'
                        }`} 
                        width={'100%'}
                        className='owner-avatar'/>
                    <span style={{fontSize: '22px', fontWeight: '600', color: '#bcbfff'}}>{discussion.owner.username}</span>
                </div>
            </div>
        ...
        <div className='disc-creation_date'>
            Дата создания: <span style={{fontWeight: 'bold'}}>{discussion.creation_date ? discussion.creation_date : '20.10.2025'}</span>
        </div> 
        ...
        <div className="topics-container">
            <span></span>
            <div className=''>
                {/* здесь будет открывашка ну и дальше додумаешься */}
            </div>
        </div>
    </div>
    )
